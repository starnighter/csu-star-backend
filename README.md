...
准备 `config.yaml` 和 `config-secret.yaml`，服务启动时会先加载 `config.yaml`，再合并 `config-secret.yaml`：

```bash
cp config-secret.production.example.yaml config-secret.yaml
```
