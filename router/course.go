...
func SetUpCourseRouter(r *gin.Engine, courseHandler *handler.CourseHandler) {
	...
	r.GET("/courses/:id/evaluations", middlewarepackage.OptionalJWTAuth(), courseHandler.GetCourseEvaluations)

	authGroup := r.Group("")
	authGroup.Use(middlewarepackage.JWTAuth())
	{
		authGroup.POST("/courses/:id/evaluations", courseHandler.CreateCourseEvaluation)
