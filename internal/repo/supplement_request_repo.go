...
type SupplementRequestItem struct {
	...
	RelatedCourseIDs    datatypes.JSON `json:"related_course_ids,omitempty"`
	RelatedCourseNames  datatypes.JSON `json:"related_course_names,omitempty"`
	RelatedTeacherIDs   datatypes.JSON `json:"related_teacher_ids,omitempty"`
...
func (r *miscRepository) ListSupplementRequests(query SupplementRequestListQuery) ([]SupplementRequestItem, int64, error) {
	...
	if keyword := strings.TrimSpace(query.Keyword); keyword != "" {
		...
		base = base.Where(`
			...
			OR supplement_requests.related_course_ids::text ILIKE ?
			OR supplement_requests.related_course_names::text ILIKE ?
			OR supplement_requests.related_teacher_ids::text ILIKE ?
...
func (r *miscRepository) baseSupplementRequestQuery() *gorm.DB {
	return r.db.Table("supplement_requests").
		...
		Select(`
			...
			supplement_requests.related_course_ids,
			supplement_requests.related_course_names,
			supplement_requests.related_teacher_ids,
